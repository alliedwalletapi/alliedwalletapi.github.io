...
if (JVM_VERSION === 2) {
	define('JPATH_VMPAYMENTPLUGIN_ALLIEDWALLET', JPATH_ROOT . DS . 'plugins' . DS . 'vmpayment' . DS . 'alliedwallet');
}
else {
	define('JPATH_VMPAYMENTPLUGIN_ALLIEDWALLET', JPATH_ROOT . DS . 'plugins' . DS . 'vmpayment');
}

class plgVMPaymentAlliedWallet extends vmPSPlugin {
	...
	function __construct (& $subject, $config) {
		...
		$this->_tablepkey = 'id'; //virtuemart_alliedwallet_id';
		$this->_tableId = 'id'; //'virtuemart_alliedwallet_id';
	...
	function getTableSQLFields () {
		...
		$SQLfields = array(
			...
			'tax_id'                           => 'smallint(1)',
			'alliedwallet_custom'                    => 'varchar(255)',
			'alliedwallet_response_payment_amount'   => 'char(15)',
			'alliedwallet_response_auth_number'      => 'char(10)',
			'alliedwallet_response_payment_currency' => 'char(3)',
			'alliedwallet_response_payment_mean'     => 'char(255)',
			'alliedwallet_response_payment_date'     => 'char(20)',
			'alliedwallet_response_payment_status'   => 'char(3)',
			'alliedwallet_response_payment_message'  => 'char(255)',
			'alliedwallet_response_card_number'      => 'char(50)',
			'alliedwallet_response_trans_id'         => 'char(6)',
			'alliedwallet_response_expiry_month'     => 'char(2)',
			'alliedwallet_response_expiry_year'      => 'char(4)',
		);
	...
	/**
	 * Prepare data and redirect to AlliedWallet payment platform
	 *
	...
	function plgVmConfirmedOrder ($cart, $order) {
		...
		if (!class_exists ('VadsApi')) {
			require(JPATH_VMPAYMENTPLUGIN_ALLIEDWALLET . DS . 'alliedwallet' . DS . 'alliedwallet_api.php');
		}
		...
		$api->set ('platform_url', $method->platform_url);
		$api->set ('MerchantID', $method->merchant_id);
		$api->set ('SiteID', $method->site_id);
		...
		$api->set ('ReturnURL', $uri->toString ());

		$url_success = JROUTE::_ (JURI::root () . 'index.php?option=com_virtuemart&view=pluginresponse&task=pluginnotification');
		$uri = JURI::getInstance ($url_success);
		//$uri->setVar('pelement', $this->payment_element);
		$uri->setVar ('Itemid', JRequest::getInt ('Itemid'));
		$uri->setVar ('pm', $order['details']['BT']->virtuemart_paymentmethod_id);
		$api->set ('ConfirmURL', $uri->toString ());
		...
		$uri->setVar ('pm', $order['details']['BT']->virtuemart_paymentmethod_id);
		$uri->setVar ('Itemid', JRequest::getInt ('Itemid'));
		$api->set ('DeclineURL', $uri->toString ());
		...
		/*$currency = $api->findCurrencyByNumCode ($currencyObj->currency_code_3);
		if ($currency == NULL) {
		...
		$api->set ('CurrencyID', $currencyObj->currency_code_3);
		...
		$api->set ('Address', $usrBT->address_1 . ' ' . $usrBT->address_2);
		...
		// Set order_id
		$api->set ('MerchantReference', $order['details']['BT']->order_number);
		$api->set ('AmountShipping', 0);
		$api->set ('NoMembership', 1);
		$api->set ('ItemName[0]', 'Product');
		$api->set ('ItemQuantity[0]', '1');
		$api->set ('ItemAmount[0]', round ($order['details']['BT']->order_total));
		$api->set ('ItemDesc[0]', 'Product');
		$api->set ('AmountTotal', round ($order['details']['BT']->order_total));
		...
		$form .= '<input type="image" name="submit" src="' . JURI::base (TRUE) . '/images/stories/virtuemart/payment/' . $this->_name . '.jpg" alt="' . JText::_ ('VMPAYMENT_' . $this->_name . '_BTN_ALT') . '" title="' . JText::_ ('VMPAYMENT_ALLIEDWALLET_BTN_ALT') . '"/>';
	...
	/**
	 * Check AlliedWallet response, save order if not done by server call and redirect to response page
	...
	// $resp->get ('order_id') is the virtuemart order_number
		$virtuemart_order_id = VirtueMartModelOrders::getOrderIdByOrderNumber ($data['MerchantReference']);

		// Order not found
		if (!$virtuemart_order_id) {
			// vmdebug('plgVmOnPaymentResponseReceived ' . $this->_name, $data, $resp->get('order_id'));
			$this->logInfo ('plgVmOnPaymentResponseReceived -- payment check attempted on non existing order : ' . $data['MerchantReference'], 'error');
...
// 	    JRequest::setVar('paymentResponseHtml', $html, 'post');
		...
		file_put_contents("testw.txt","order".print_r($order, true));
		$order_status_code = $order['items'][0]->order_status;
			...
			$amount = $order['details']['BT']->order_total . ' ' . $currency;
			$html = $this->_getHtmlPaymentResponse ('VMPAYMENT_' . $this->_name . '_SUCCESS_MSG', TRUE, $virtuemart_order_id, $amount);
			//JRequest::setVar('paymentResponseHtml', $html, 'post');
...
//We delete the old stuff
	...
	/**
	 * Process a AlliedWallet payment cancellation.
	 *
	 * @param int $virtuemart_order_id virtuemart order primary key concerned by payment
	 * @return
	 */
	...
	/**
	 * Check AlliedWallet response, save order and empty cart (if payment success) when server notification is received from payment platform.
	 *
	...
	function plgVmOnPaymentNotification () {
		...
		if ($order_status_code == 'P') {
			...
			$this->managePaymentResponse ($virtuemart_order_id, $resp, $new_status, $return_context, $data['order_id']);
		}
		else {
