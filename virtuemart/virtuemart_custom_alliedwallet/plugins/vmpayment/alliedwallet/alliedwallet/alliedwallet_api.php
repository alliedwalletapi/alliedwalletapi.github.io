...
/*
  ...
  #									########################
  #					Developed By
  #						Contact : support@alliedwallet.com
  #						Developer : amanit007@gmail.com
  #####################################################################################################
 */
...
/**
 * @package VadsApi
 * @author Prateek/Aman <support@alliedwallet.com>
 * @copyright alliedwallet.com
 * PHP classes to integrate an e-commerce solution with the payment platform supported by alliedwallet.
 */
...
class VadsApi {
	...
	function VadsApi($encoding="UTF-8") {
		...
		$this->_addRequestField('AmountTotal', 'Amount', '#^' . $supzero . '$#',
						true);
		$this->_addRequestField('available_languages', 'Available languages',
						"#^(|[A-Za-z]{2}(;[A-Za-z]{2})*)$#", false, 2);
		$this->_addRequestField('capture_delay', 'Capture delay', "#^\d*$#");
		$this->_addRequestField('contracts', 'Contracts', $ans255);
		$this->_addRequestField('contrib', 'Contribution', $ans255);
		$this->_addRequestField('ctx_mode', 'Mode', "#^TEST|PRODUCTION$#",
						true);
		$this->_addRequestField('CurrencyID', 'Currency', "#^\d{3}$#", true, 3);
		$this->_addRequestField('cust_antecedents', 'Customer history',
						"#^NONE|NO_INCIDENT|INCIDENT$#");
		$this->_addRequestField('cust_address', 'Customer address', $ans255);
		$this->_addRequestField('cust_country', 'Customer country',
						"#^[A-Za-z]{2}$#", false, 2);
		$this->_addRequestField('cust_email', 'Customer email', $regex_mail,
						false, 127);
		$this->_addRequestField('cust_id', 'Customer id',
						$an63, false, 63);
		$this->_addRequestField('cust_name', 'Customer name',
						$ans127, false, 127);
		$this->_addRequestField('cust_cell_phone', 'Customer cell phone',
						$an63, false, 63);
		$this->_addRequestField('cust_phone', 'Customer phone', $an63, false,
						63);
		$this->_addRequestField('cust_title', 'Customer title', '#^'.$ans.'{0,63}$#', false,
						63);
		$this->_addRequestField('cust_city', 'Customer city',
						'#^' . $ans . '{0,63}$#', false, 63);
		$this->_addRequestField('cust_state', 'Customer state/region', '#^'.$ans.'{0,63}$#', false,
						63);
		$this->_addRequestField('cust_zip', 'Customer zip code', $an63, false,
						63);
		$this->_addRequestField('order_id', 'Order id',
						"#^[A-za-z0-9]{0,12}$#", false, 12);
		$this->_addRequestField('order_info', 'Order info', $ans255);
		$this->_addRequestField('order_info2', 'Order info 2', $ans255);
		$this->_addRequestField('order_info3', 'Order info 3', $ans255);
		$this->_addRequestField('payment_cards', 'Payment cards',
						"#^[A-Za-z0-9;]{0,127}$#", false, 127);
		$this->_addRequestField('payment_src', 'Payment source', "#^$#", false,
						0);
		$this->_addRequestField('redirect_error_message',
						'Redirection error message', $ans255, false);
		$this->_addRequestField('redirect_error_timeout',
						'Redirection error timeout', $ans255, false);
		$this->_addRequestField('redirect_success_message',
						'Redirection success message', $ans255, false);
		$this->_addRequestField('redirect_success_timeout',
						'Redirection success timeout', $ans255, false);
		$this->_addRequestField('return_mode', 'Return mode',
						"#^NONE|GET|POST?$#", false, 4);
		$this->_addRequestField('return_get_params', 'GET return parameters',
						$regex_params, false);
		$this->_addRequestField('return_post_params',
						'POST return parameters', $regex_params, false);
		$this->_addRequestField('ship_to_name', 'Shipping name',
						'#^' . $ans . '{0,127}$#', false, 127);
		$this->_addRequestField('ship_to_phone_num', 'Shipping phone',
						$ans255, false, 63);
		$this->_addRequestField('ship_to_street', 'Shipping street', $ans127,
						false, 127);
		$this->_addRequestField('ship_to_street2', 'Shipping street (2)',
						$ans127, false, 127);
		$this->_addRequestField('ship_to_state', 'Shipping state', $an63,
						false, 63);
		$this->_addRequestField('ship_to_country', 'Shipping country',
						"#^[A-Za-z]{2}$#", false, 2);
		$this->_addRequestField('ship_to_city', 'Shipping city',
						'#^' . $ans . '{0,63}$#', false, 63);
		$this->_addRequestField('ship_to_zip', 'Shipping zip code', $an63,
						false, 63);
		$this->_addRequestField('shop_name', 'Shop name', $ans127);
		$this->_addRequestField('shop_url', 'Shop url', $ans127);
		$this->_addRequestField('MerchantID', 'Merchant id', "#^\d{8}$#", true, 8);
		$this->_addRequestField('SiteID', 'Site id', "#^\d{8}$#", true, 8);
		$this->_addRequestField('MerchantReference', 'Merchant Reference', "#^\d{8}$#", true, 8);
		$this->_addRequestField('AmountShipping', 'Shipping Amount', "#^\d{8}$#", true, 8);
		$this->_addRequestField('NoMembership', 'Membership', "#^\d{8}$#", true, 8);
		$this->_addRequestField('ItemName[0]', 'Membership', "#^\d{8}$#", true, 8);
		$this->_addRequestField('ItemQuantity[0]', 'Membership', "#^\d{8}$#", true, 8);
		$this->_addRequestField('ItemDesc[0]', 'Membership', "#^\d{8}$#", true, 8);
		$this->_addRequestField('ItemAmount[0]', 'Membership', "#^\d{8}$#", true, 8);
		$this->_addRequestField('theme_config', 'Theme', $ans255);
		$this->_addRequestField('ConfirmURL', 'Success url', $ans127, false,
						127);
		$this->_addRequestField('url_referral', 'Referral url', $ans127,
						false, 127);
		$this->_addRequestField('url_refused', 'Refused url', $ans127, false,
						127);
		$this->_addRequestField('DeclineURL', 'Cancel url', $ans127, false,
						127);
		$this->_addRequestField('url_error', 'Error url', $ans127, false, 127);
		$this->_addRequestField('ReturnURL', 'Return url', $ans127, false,
						127);
		
		// Enable / disable 3D Secure
		//$this->_addRequestField('threeds_mpi', 'Enable / disable 3D Secure', '#^[0-2]$#', false);

		// Declaration of parameters for Oney payment
		$this->_addRequestField('cust_first_name', 'Customer first name', $an63, false, 63);
		$this->_addRequestField('cust_last_name', 'Customer last name', $an63, false, 63);
		$this->_addRequestField('cust_status', 'Customer status (private or company)', "#^PRIVATE|COMPANY$#", false, 7);

		$this->_addRequestField('ship_to_first_name', 'Shipping first name', $an63, false, 63);
		$this->_addRequestField('ship_to_last_name', 'Shipping last name', $an63, false, 63);
		$this->_addRequestField('ship_to_status', 'Shipping status (private or company)', "#^PRIVATE|COMPANY$#", false, 7);
		$this->_addRequestField('ship_to_delivery_company_name', 'Name of the delivery company', $ans127, false, 127);
		$this->_addRequestField('ship_to_speed', 'Speed of the shipping method', "#^STANDARD|EXPRESS$#", false, 8);
		$this->_addRequestField('ship_to_type', 'Type of the shipping method',
						"#^RECLAIM_IN_SHOP|RELAY_POINT|RECLAIM_IN_STATION|PACKAGE_DELIVERY_COMPANY|ETICKET$#", false, 24);

		$this->_addRequestField('insurance_amount', 'The amount of insurance', '#^' . $supzero . '$#', false, 12);
		$this->_addRequestField('tax_amount', 'The amount of tax', '#^' . $supzero . '$#', false, 12);
		$this->_addRequestField('shipping_amount', 'The amount of shipping', '#^' . $supzero . '$#', false, 12);
		$this->_addRequestField('nb_products', 'Number of products', '#^' . $supzero . '$#', false);
	...
	/**
	 * General getter.
	 * Retrieve an api variable from its name. Automatically add '' to the name if necessary.
	...
	function get($name) {
		...
		$name = (substr($name, 0, 5) != '') ? '' . $name : $name;

		if ($name == 'key_test') {
			return $this->keyTest;
		} elseif ($name == 'key_prod') {
			return $this->keyProd;
		} elseif ($name == 'platform_url') {
			return $this->platformUrl;
		} elseif ($name == 'redirect_enabled') {
	...
	/**
	 * General setter.
	 * Set an api variable with its name and the provided value. Automatically add '' to the name if necessary.
	...
	*/
	function addProduct($label, $amount, $qty, $ref, $type) {
		...
		// Add product infos as request parameters
		$ok &= $this->_addRequestField("product_label" . $index, "Product label", '#^[^<>"+-]{0,255}$#', false, 255, $label);
		$ok &= $this->_addRequestField("product_amount" . $index, "Product amount", '#^[1-9]\d*$#', false, 12, $amount);
		$ok &= $this->_addRequestField("product_qty" . $index, "Product quantity", '#^[1-9]\d*$#', false, 255, $qty);
		$ok &= $this->_addRequestField("product_ref" . $index, "Product reference", '#^[A-Za-z0-9]{0,64}$#', false, 64, $ref);
		$ok &= $this->_addRequestField("product_type" . $index, "Product type", "#^".implode("|", $this->ACCORD_CATEGORIES)."$#",
			false, 30, $type);
	...
	function getRequestHtmlForm($enteteAdd = '', $inputType = 'hidden',
			$buttonValue = 'AlliedWallet Payment', $buttonAdd = '',
			$buttonType = 'submit') {
...
class VadsResponse {
	...
	var $code;
	/**
	 * Translation of $code (result)
	 * @var string
	 * @access private
	 */
	var $message;
	/**
	 * Value of extra_result
	 * @var string
	 * @access private
	 */
	var $extraCode;
	/**
	 * Translation of $extraCode (extra_result)
	 * @var string
	 * @access private
	 */
	var $extraMessage;
	/**
	 * Value of auth_result
	 * @var string
	 * @access private
	 */
	var $authCode;
	/**
	 * Translation of $authCode (auth_result)
	 * @var string
	 * @access private
	 */
	var $authMessage;
	/**
	 * Value of warranty_result
	 * @var string
	 * @access private
	 */
	var $warrantyCode;
	/**
	 * Translation of $warrantyCode (warranty_result)
	 * @var string
	 * @access private
	 */
	var $warrantyMessage;
	...
	function load($raw, $certificate) {
		...
		// Get codes
		$code = $this->_findInArray('result', $raw, null);
		$extraCode = $this->_findInArray('extra_result', $raw, null);
		$authCode = $this->_findInArray('auth_result', $raw, null);
		$warrantyCode = $this->_findInArray('warranty_code', $raw, null);
	 ...
	 */
	function get($name) {
		// Manage shortcut notations by adding ''
