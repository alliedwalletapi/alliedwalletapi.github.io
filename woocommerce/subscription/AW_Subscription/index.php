...
add_action('plugins_loaded', 'woocommerce_aw_sub_init', 0);
function woocommerce_aw_sub_init(){
  if(!class_exists('WC_Payment_Gateway')) return;
 
  class WC_aw_sub extends WC_Payment_Gateway{
    public function __construct(){
      ...
      $this -> id = 'aw_sub';
      $this -> method_title = 'AlliedWalletSubscription - Nextgen';
      ...
      $this -> userid = $this -> settings['token'];
      $this -> priceID = $this -> settings['priceID'];
      $this -> redirect_page_id = $this -> settings['redirect_page_id'];
      $this -> icon = apply_filters('woocommerce_aw_sub_icon', plugin_dir_url(__FILE__) . '/images/alliedwallet.png');
      ...
      add_action('woocommerce_receipt_aw_sub', array(&$this, 'receipt_page'));
    ...
    function init_form_fields(){
       ...
       $this -> form_fields = array(
                'enabled' => array(
                    ...
                    'type' => 'checkbox',
                    'label' => __('Enable AlliedWallet Payment Module.', 'aw_sub'),
                    'default' => 'no'),
                'title' => array(
                    'title' => __('Title:', 'aw_sub'),
                    'type'=> 'text',
                    'description' => __('This controls the title which the user sees during checkout.', 'aw_sub'),
                    'default' => __('aw_sub', 'aw_sub')),
                'description' => array(
                    'title' => __('Description:', 'aw_sub'),
                    'type' => 'textarea',
                    'description' => __('This controls the description which the user sees during checkout.', 'aw_sub'),
                    'default' => __('Pay securely by Credit or Debit card or internet banking through AlliedWallet.', 'aw_sub')),
                'merchant_id' => array(
                    'title' => __('Merchant ID', 'aw_sub'),
                    'type' => 'text',
                    'description' => __('This MID is issued from AW.')),
                'site_id' => array(
                    'title' => __('Site ID', 'aw_sub'),
                    'type' => 'text',
                    'description' => __('This SID is issued from AW.')),
                'token' => array(
                    'title' => __('QuickPayToken', 'aw_sub'),
                    'type' => 'text',
                    'description' => __('This is the token issued from AW.')),
                 'priceID' => array(
                    'title' => __('SubscriptionPlanId', 'aw_sub'),
                    'type' => 'text',
                    'description' => __('This is the priceID issued from AW.')),
                'debug' => array(
								'title' => __( 'Debug Log', 'woocommerce' ), 
								'type' => 'checkbox', 
								'label' => __( 'Enable logging', 'woocommerce' ), 
								'default' => 'no',
								'description' => __( 'Log Allied Wallet events, such as pingback requests, inside <code>woocommerce/logs/aw_sub.txt</code>' ),
							)
       ...
       public function admin_options(){
        echo '<h3>'.__('AW Payment Gateway', 'aw_sub').'</h3>';
    ...
    function receipt_page($order){
        echo '<p>'.__('Thank you for your order, please click the button below to pay with AW.', 'aw_sub').'</p>';
        echo $this -> generate_aw_sub_form($order);
    }
    /**
     * Generate aw button link
     **/
    public function generate_aw_sub_form($order_id){
 ...
 /**
		...
		function check_postback_request_is_valid() {
	        ...
	        if ($this->debug=='yes') $this->log->add( 'aw_sub', 'Pingback Response: ' . print_r($response, true) );
	        
	        // check to see if the request was valid
	        if ( !is_wp_error($response) && isset($response['TrackingId']) && isset($response['TransactionID']) ) {
	            if ($this->debug=='yes') $this->log->add( 'aw_sub', 'Received valid response from Allied Wallet' );
	            return true;
	        }  
	        
	        if ($this->debug=='yes') :
	        	$this->log->add( 'aw_sub', 'Received invalid response from Allied Wallet' );
	        	if (is_wp_error($response)) :
	        		$this->log->add( 'aw_sub', 'Error response: ' . $result->get_error_message() );
	        	endif;
	        endif;
 ...
 /**
		...
		function check_pingback_response() {
			...
			if (isset($_GET['aw_subListener']) && $_GET['aw_subListener'] == 'aw_sub_pingback'):
	        	...
	        	if ($this->check_postback_request_is_valid()) :
	            	...
	            	do_action("valid-aw-sub-pingback-request", $_POST);
				
				else :
				
			        if ($this->debug=='yes') $this->log->add( 'aw_sub', 'Allied Wallet Pingback Failure' );
		...
		function handle_callback( ) {
		    ...
		    if ( !empty($response['TrackingId']) && !empty($response['Amount']) ) {
		        ...
		        if ($this->debug=='yes') $this->log->add( 'aw_sub', 'Processing Pingback for Merchant Reference: ' . print_r($reference, true));		
		        ...
		        if ($order->order_key !== $order_key) :
		        	if ($this->debug=='yes') $this->log->add( 'aw_sub', 'Error: Order Key does not match invoice.' );
		        	exit;
		        endif;
				...
				if ( isset($response['TrackingId']) && isset($response['TransactionID']) && $response['TransactionStatus']=="Successful") {
	                ...
	                if ($this->debug=='yes') $this->log->add( 'aw_sub', 'Payment complete.' );
...
/**
     ...
     **/
    function woocommerce_add_aw_sub_gateway($methods) {
        $methods[] = 'WC_aw_sub';
        return $methods;
    }
 
    add_filter('woocommerce_payment_gateways', 'woocommerce_add_aw_sub_gateway' );
