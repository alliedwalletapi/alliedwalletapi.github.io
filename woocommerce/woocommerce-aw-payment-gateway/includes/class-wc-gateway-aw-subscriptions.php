...
/**
 * WC_Gateway_Aw_Subscriptions class.
 *
 * @since 2.0.0
 * @extends WC_Payment_Gateway
 */
class WC_Gateway_Aw_Subscriptions extends WC_Gateway_Aw {
	...
	public function scheduled_subscription_payment( $amount_to_charge, $renewal_order ) {
		...
		if ( is_wp_error( $response ) ) {
			$renewal_order->update_status( 'failed', sprintf( __( 'Aw Transaction Failed (%s)', 'woocommerce-gateway-aw' ), $response->get_error_message() ) );
		}
	}
	...
	public function process_subscription_payment( $order, $amount = 0 ) {
		...
		if( substr( $id, 0, 1 ) !== '_' ) {
			...
			$aw_request['customer_vault_id'] = $user->user_login;
			$aw_request['billing_id']        = substr( $id , 1 );
			$aw_request['ver']               = 2;
		}
		...
		if ( $response['response'] == 1 ) {
			// Success
			$order->add_order_note( __( 'Aw Commerce scheduled subscription payment completed. Transaction ID: ' , 'woocommerce-gateway-aw' ) . $response['transactionid'] );
			WC_Subscriptions_Manager::process_subscription_payments_on_order( $order );

		} else if ( $response['response'] == 2 ) {
			// Decline
			$order->add_order_note( __( 'Aw Commerce scheduled subscription payment failed. Payment declined.', 'woocommerce-gateway-aw') );
			WC_Subscriptions_Manager::process_subscription_payment_failure_on_order( $order );

		} else if ( $response['response'] == 3 ) {
			// Other transaction error
			$order->add_order_note( __( 'Aw Commerce scheduled subscription payment failed. Error: ', 'woocommerce-gateway-aw') . $response['responsetext'] );
			WC_Subscriptions_Manager::process_subscription_payment_failure_on_order( $order );

		} else {
			// No response or unexpected response
			$order->add_order_note( __('Aw Commerce scheduled subscription payment failed. Couldn\'t connect to gateway server.', 'woocommerce-gateway-aw') );
